...
(function () {
  ...
  function renderProfile() {
    ...
    if (currentView === 'fused') {
      ...
      const divId = 'chart-card-fused';
      const wrap  = document.createElement('div');
      wrap.className = 'card section-card';
      wrap.innerHTML = `<h2 class="section-title">${escHtml(title)}</h2><div id="${divId}"></div>`;
      container.appendChild(wrap);

      const fig = cardProfileFused(profile, enabledModels, currentCard, currentRole, title);
      Plotly.react(divId, fig.data, fig.layout, CHART_CONFIG);
    } else {
      enabledModels.forEach(n => {
        const title = `${currentCard} as ${roleLabel} — ${n}`;
        const divId = `chart-card-sep-${n.replace(/[^a-zA-Z0-9]/g, '_')}`;
        const wrap  = document.createElement('div');
        wrap.className = 'card section-card';
        wrap.innerHTML = `<h2 class="section-title">${escHtml(`${currentCard} vs ${n}`)}</h2><div id="${divId}"></div>`;
        container.appendChild(wrap);

        const fig = cardProfileSeparate(profile, n, currentCard, currentRole, title);
        Plotly.react(divId, fig.data, fig.layout, CHART_CONFIG);
      });
    }
  }
